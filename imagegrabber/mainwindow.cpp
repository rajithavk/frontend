...
void GrabberWindow::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event)
    QSettings settings;
...
void GrabberWindow::on_pushButtonSettings_clicked()
{
    settingsDialog.show();
}


void GrabberWindow::newImageReceieved(QImage *img)
{
    ui->graphicsViewImage->newImageReceieved(img);
}

void GrabberWindow::newFPSCalculated(double fps, double avg)
{
...
void GrabberWindow::on_comboBoxGrabberTypes_activated(int index)
{
    setImageGrabber(imageGrabbers.at(index));
}

void GrabberWindow::setImageGrabber(QImageGrabber *gb)
{
...
void GrabberWindow::on_pushButtonStart_clicked()
{
    if (currentGrabber != NULL) {
...
void GrabberWindow::grabbingStateChanged(QImageGrabber::GrabbingState newState)
{
