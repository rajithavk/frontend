...
display::display(QWidget *parent, multimap<int, QPixmap> *pics): QMainWindow(parent), ui(new Ui::display){
    ui->setupUi(this);
    ...
    map<int,QPixmap>::iterator it = pics->begin();
...
void display::butgrid(const QString &passed){
    dbc.setDatabaseName(Path_to_DB);
        multimap<int, QPixmap> pics_temp;
